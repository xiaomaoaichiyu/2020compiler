...
extern vector<map<string, vector<CodeItem>>> func2tmpvar2Codes;
...
private:
	...
	std::vector<int> globalRegAllocated;	// 函数内需要的全局寄存器数量
	std::vector<std::map<std::string, bool>> paraIfNeed;	// 记录函数内参数是否被重新定义过
